...
if($act=="diagnosa"){
	...
	$qryg = mysqli_query($GLOBALS["___mysqli_ston"], $sqlg);
...
<?php
$u=$_SESSION['SESS_USERNAME'];
$cek_induk=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM gejala where  kode_induk_ya='$induk'"));
$cek_simpul=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM gejala where  kode_induk_tidak='$s'"));
$sql_cekh = "SELECT * FROM tmp_penyakit 
			 WHERE username='$u' 
			 GROUP BY kode_penyakit";
$qry_cekh = mysqli_query($GLOBALS["___mysqli_ston"], $sql_cekh);
$hsl_cekh = mysqli_num_rows($qry_cekh);
if ($cek_induk == 0 or $cek_simpul==0) {
	
	$hsl_data = mysqli_fetch_array($qry_cekh);
	$cek_gejala_valid=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tmp_gejala where status='1'"));
	$cek_gejala_penyakit=mysqli_num_rows(mysqli_query($GLOBALS["___mysqli_ston"], "SELECT relasi_penyakit_gejala.* FROM relasi_penyakit_gejala,tmp_penyakit where relasi_penyakit_gejala.kode_penyakit=tmp_penyakit.kode_penyakit"));
	$hasilbobot= mysqli_query($GLOBALS["___mysqli_ston"], "SELECT bobot
...
AND tmp_gejala.status =1");
	...
	if ($persentase==0){
	    ...
	    $qry_pasien = mysqli_query($GLOBALS["___mysqli_ston"], $sql_pasien);
...
function AddTmpAnalisa($kode_gejala, $u) {
	...
	$qry_sakit = mysqli_query($GLOBALS["___mysqli_ston"], $sql_sakit);
	while ($data_sakit = mysqli_fetch_array($qry_sakit)) {
		...
		mysqli_query($GLOBALS["___mysqli_ston"], $sqltmp);
	}
}
...
function AddTmpGejala($kode_gejala, $u,$status) {
	...
	mysqli_query($GLOBALS["___mysqli_ston"], $sql_gejala);
}
...
if ($jawaban == "Y") {
	...
	$qry_analisa = mysqli_query($GLOBALS["___mysqli_ston"], $sql_analisa);
	...
	if ($data_cek >= 1) {
		...
		$sql_tmp = "SELECT * FROM tmp_analisa 
					...
					AND username='$u'";
		$qry_tmp = mysqli_query($GLOBALS["___mysqli_ston"], $sql_tmp);
		while ($data_tmp = mysqli_fetch_array($qry_tmp)) {
			$sql_rsakit = "SELECT * FROM relasi_penyakit_gejala 
							...
							GROUP BY kode_penyakit";
			$qry_rsakit = mysqli_query($GLOBALS["___mysqli_ston"], $sql_rsakit);
			while ($data_rsakit = mysqli_fetch_array($qry_rsakit)) {
				...
				mysqli_query($GLOBALS["___mysqli_ston"], $sql_input);
			}
		} 
		// Gunakan Fungsi
		DelTmpAnlisa($u);
		...
		$qry_rgejala = mysqli_query($GLOBALS["___mysqli_ston"], $sql_rgejala);
		while ($data_rgejala = mysqli_fetch_array($qry_rgejala)) {
			...
			while ($data_rsakit = mysqli_fetch_array($qry_rsakit)) {
				// Data penyakit yang mungkin dimasukkan ke tmp
				$sql_input = mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tmp_penyakit (username,kode_penyakit)
							 VALUES ('$u','$data_rsakit[kode_penyakit]')");
			}
		} 
		// Menggunakan Fungsi
...
if ($act=="hasil"){
$u=$_SESSION['SESS_USERNAME'];
$qry = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM hasil_diagnosa, penyakit, data_user WHERE penyakit.kode_penyakit=hasil_diagnosa.kode_penyakit AND hasil_diagnosa.username='$u' AND hasil_diagnosa.username=data_user.username ORDER BY hasil_diagnosa.id_diagnosa DESC LIMIT 1");
$data = mysqli_fetch_array($qry);
$id = $data['id_diagnosa'];
mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE TABLE `tmp_analisa`");
mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE TABLE `tmp_gejala`");
mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE TABLE `tmp_penyakit`");
?>
<div class="text-area-user" align="justify">  
<br>
...
<tr>
  ...
  <tr>
    <td><div align="right"><strong>Tipe Penyakit Kulit</strong></div></td>
    <td>:</td>
...
if ($act=="hasil0"){
$u=$_SESSION['SESS_USERNAME'];
$qry = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM hasil_diagnosa, data_user WHERE hasil_diagnosa.username='$u' AND hasil_diagnosa.username=data_user.username ORDER BY hasil_diagnosa.id_diagnosa DESC LIMIT 1");
$data = mysqli_fetch_array($qry);
$id = $data['id_diagnosa'];
mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE TABLE `tmp_analisa`");
mysqli_query($GLOBALS["___mysqli_ston"], "TRUNCATE TABLE `tmp_gejala`");
...
<tr>
  ...
  <tr>
    ...
    <td valign="top">:</td>
    <td valign="top">Hasil kesimpulan diagnosa, anda tidak mengalami jenis penyakit kulit apapun, disebabkan tidak ada gejala yang anda alami.</td>
  </tr>
