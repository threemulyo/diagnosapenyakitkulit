...
<tr>
  ...
  <tr>
    <td class="subtitle">Tipe Penyakit Kulit</td>
  </tr>
  <tr>
    <td>
      <select name="jumpMenu" id="jumpMenu" onChange="MM_jumpMenu('parent',this,0)">
        <option value="?page=6&act=relasi&kode_penyakit" selected="selected">[ Tipe Penyakit Kulit ]</option>
        <?php
				$qryp = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM penyakit");
  ...
  <tr>
	...
	<?php
	$no=0;
  $qry = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM gejala ORDER BY kode_gejala");
	while ($data=mysqli_fetch_array($qry)){
		$no++;
		$qryr = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM relasi_penyakit_gejala WHERE kode_penyakit='$kode_penyakit' AND kode_gejala='$data[kode_gejala]'");
...
elseif($act=="simpanrelasi"){
	...
	if($jum==0){
		...
		$qpil = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM relasi_penyakit_gejala WHERE kode_penyakit='$kode_penyakit'");
		while($datapil = mysqli_fetch_array($qpil)){
			for($i = 0; $i < $jum; ++$i){
				if($datapil['kode_gejala'] != $cekgejala[$i]){
					mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM relasi_penyakit_gejala WHERE kode_penyakit='$kode_penyakit' AND NOT kode_gejala IN('$cekgejala[$i]')");
				}
			}
		}
		for($i = 0; $i < $jum; ++$i){
			$qryr = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM relasi_penyakit_gejala WHERE kode_penyakit='$kode_penyakit' AND kode_gejala='$cekgejala[$i]'");
			$cocok = mysqli_num_rows($qryr);
			if(!$cocok==1){
				mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO relasi_penyakit_gejala(kode_penyakit, kode_gejala) VALUES('$kode_penyakit', '$cekgejala[$i]')");
				$gejala = $cekgejala[$i].",";
